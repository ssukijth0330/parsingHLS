...
pub fn from_secs_f32(url: &str) {
    //read the url from user input string. 
    ...
    for line in lines {
        ...
        if line.contains("#EXTINF") {
            ...
            // then convert the string to f32 format
    ...
    println!("The duration to play all segments of this video is(secs): {}\n", duration);
    // println!("Note:
...
// bigb_720.m3u8
//
// The code will print the duration of bigb_480.m3u8 and bigb_720.m3u8
//
...
//funcion to combine the base URL with the file name that we found in the m3u8 file
//Input: the original URL and the file name that we found in the m3u8 file
//Output: the combined URL
pub fn replace_with_newfilename_in_url(in_url: &str, m3u8_filename: &str) -> String {
    ...
    //combine the base URL with path and the file name that we found in the m3u8 file
